...
**Pelada Semanal**, beach tennis **feminino** na **a quadra**, **toda semana,
20h**. A lista de confirmação abre na semana da semana anterior; R$ 45 por
participação, com check-in por Wellhub/Gympass e TotalPass acertado com a
administradora do grupo.

Não confundir com "Play das Jogadores" — esse é um campeonato **concorrente**, e
...
nas chaves do `localStorage`. Tudo renomeado para `play-da-semana`.

Este repositório nasceu como clone do app do campeonato feminino. O nome
**Pelada Semanal** é rótulo de trabalho: o grupo ainda não escolheu o dele.
...
**quantos jogadores vieram** (os jogadores decidem durante a semana) e **quantas
quadras** temos.
...
fila a partida cujas quatro jogadores estão livres (`proximasDasQuadras`).
...
- **A escolha é do conjunto, não quadra por quadra.** Pegar a melhor partida
  para a quadra 1 sem olhar os outros fazia a mesmo jogador ser sugerida em duas
  quadras ao mesmo tempo (visto no teste com 16 jogadores). Agora uma busca
...
- **Quando não dá para preencher, o app fala.** Em vez de sugerir alguém que já
  ...
  jogadores livres", lista quem está livre e oferece montar uma partida com elas.

## Duplas: parceiro, adversária e descanso

As duplas são montadas pela pontuação acumulada, para os jogos ficarem parelhos.

- O rodízio usa o **método do círculo** (1-fatoração): fixa um jogador e gira
  os outros, gerando "rondas" de duplas em que ninguém se repete. Toda
  combinação de parceiros aparece **exatamente uma vez**.
- **Quem enfrenta quem também é escolhido** — antes era sobra do sorteio, e dois
  jogadores podiam se enfrentar 5 vezes enquanto outras nunca se cruzavam.
...
- **Duplas órfãs, corrigido.** Quando a ronda tem um número ímpar de duplas,
  ...
  da mesmo jogador, e aí elas não conseguiam se enfrentar depois: 3 duplas
  repetidas onde a matemática exige 1. Agora sai a dupla cujas jogadores menos
  aparecem nos outros sobras, e o resultado bate o mínimo teórico: **0 duplas
...
- **A otimização de confrontos só troca duplas DENTRO da mesma ronda.** Trocar
  ...
  Medido em 16 jogadores e 3 quadras: **quadra ociosa em 11% dos casos com a
  restrição contra 46% sem ela**. A restrição fica.
- **Entra sempre quem está fora há mais tempo.** Sem isso acontecia de o jogador
...
- **Empate na espera tem que dar a MESMA posição** (`ordemDeEspera` usa
  ...
  Medido no app com 20 jogadores e 3 quadras: **antes a quadra livre chamava
  alguém que tinha acabado de sair, com 8 jogadores ainda esperando a primeira
...
jogadores que nem tinham entrado ficavam lá embaixo — e contradizia o que as
...
## Todos jogam o mesmo tanto, mesmo quando a conta não fecha
...
joga de novo. Medido, isso dava **um jogo a mais para exatamente dois jogadores**:

| grupo | antes | depois |
|---|---|---|
| 6 | 8 partidas — 2 fazem 6 jogos, 4 fazem 5 | **9 partidas — todos fazem 6** |
| 7 | 11 — 2 fazem 7, 5 fazem 6 | **14 — todos fazem 8** |
| 10 | 23 — 2 fazem 10, 8 fazem 9 | **25 — todos fazem 10** |
| 11 | 28 — 2 fazem 11, 9 fazem 10 | **33 — todos fazem 12** |
...
perfeito quando cada um repete 1 (grupo par), um ciclo passando por todas
...
A folga de um grupo é `tamanho % 4`: quantas ficam de fora enquanto os outros
...
os outros"), e com **grupos de 4 isso é falso**: cada grupo enche exatamente uma
quadra, então a quadra não espera ninguém — a próxima partida é das mesmas
quatro jogadores, que jogam as 3 partidas seguidas sem parar. O problema existe,
...
grupo) e a recomendação em grupos aponta para 5/9/13, não para "+4 jogadores",
...
para um jogador e a menos para outra — exatamente o que a regra de todas
...
jogadores` sem grupos, que já tem aviso próprio na criação, e cuja solução é
usar uma quadra a menos, não mexer nas duplas.

A troca continua existindo para o que ela realmente serve: **alguém foi embora**.
Aí a organizador troca o jogador na mão, no card da partida, escolhendo quem
...
Cada partida precisa de **quatro jogadores do mesmo grupo**. Então um grupo de 6
só enche **uma** quadra por vez, por mais gente que tenha no play: 12 jogadores
...
"12 jogadores em 3 quadras, ninguém fica de fora" — errado nas duas metades — e
...
agora recebe os grupos e só aceito substituto do mesmo grupo da partida; o botão
...
Descoberto na simulação: **quando quadras × 4 = número de jogadores, ninguém sobra
e a quadra que terminar primeiro fica parada**, porque as quatro da próxima
partida ainda estão jogando. Com 16 jogadores e 4 quadras a quadra fica ociosa
...
sozinho. A configuração ideal medida foi **24 jogadores em 3 grupos de 8 com 3
quadras: 0% de quadra ociosa e 11 minutos de descanso médio**.

## Modo em grupos

Com 16 jogadores o rodízio completo dá **60 partidas e 15 jogos para cada um** —
longo demais para uma noite. O modo em grupos resolve: o mesmo rodízio acontece
**dentro de cada grupo**, então grupos de 8 dão 7 jogos por jogador.
...
A força de cada jogador alimenta duas coisas: o equilíbrio das duplas e a
...
**3. Elo (o atual).** Cada partida move a nota das quatro jogadores conforme a
...
Medido em 12 semanas simuladas, 16 jogadores, 30 temporadas — correlação entre a
nota e a habilidade real (1,00 = perfeita):

| | semana 2 | semana 4 | semana 8 | semana 12 |
|---|---|---|---|---|
| **todos com todos** — média de pontos | 0,85 | 0,91 | 0,90 | 0,91 |
| **todos com todos** — Elo | 0,88 | 0,93 | 0,95 | **0,96** |
...
> para ela — mas aí um jogador forte passa algumas semanas em jogos fáceis.

## Play avulso

Nem todo jogo é campeonato — os jogadores marcam um play numa segunda qualquer.
Na criação dá para desmarcar **"vale para o campeonato"** (`sessions.ranked`).

O play avulso **conta** no histórico de cada jogador, nas estatísticas e na
força que equilibra as duplas dos próximos plays. Ele **não** soma pontos no
ranking do mês e **não** mexe nas sequências 🔥 — o status é sobre as semanas.
...
A pergunta difícil foi: **o que mantém o status?** Ganhar a semana seguidas vezes
...
> apontou: se todos jogam com todas, as melhores sobem ao pódio mesmo.
>
> O raciocínio falho era supor que o app equilibra as **duplas**. Ele não pode:
> no rodízio completo o parceiro de cada partida já está determinada, e ao fim
> do dia **todo mundo joga com todo mundo uma vez** — a soma da força das
> parceiros é idêntica para todas. O que o app escolhe é só a **adversária**.
>
> Medido com o algoritmo atual (600 semanas simuladas, 16 jogadores, forças
> espalhadas de 0,8 a 3,2 pontos por partida):
>
> | | o melhor jogador | acaso puro |
> |---|---|---|
...
da quadra fica no pódio toda semana** (fica fora em 3 a 5 de cada 10 semanas) e
**o pódio inteiro quase nunca é o top 3 real** (8% das vezes). Emendar várias
semanas seguidas continua sendo difícil: o campeão de um dia repete no dia
...
Que os grupos são justos entre si já estava medido (800 plays, 16 jogadores):

| | grupo 1 (mais fortes) | grupo 2 |
|---|---|---|
| vagas no pódio | 48% | 52% |
| campeão do dia | 47% | 53% |
| pontos por jogador | 7,7 | 7,8 |
...
81% dos jogadores encostam nele em 16 semanas, contra 33%. É o degrau mais barato
(3 pontos), então foi aceito de propósito: em modo grupos o 🔥 vira convite, e
os títulos de verdade continuam onde estavam.

Em pontos o efeito é pequeno: no fechamento do mês há **2,4** jogadores com
...
organizador escolher sabendo o que está fazendo.
...
- termina a semana no top 3 → mantém e sobe o status;
- **faltar na semana zera o status, mesmo tendo vida** (regra explícita do dono);
- a vida absorve **uma** semana fora do pódio, e é ganha ao preservar no fim do mês.
...
🔥🔥🔥 Imparável (4) · 👑🔥 Lenda da pelada (5) · 👑💎 Rei da quadra (6) ·
👑🌟 Imperador (7) · 👑💎🌟 **Craque da temporada** (8+, o teto).
...
o mês. No fechamento o jogador escolhe **usar** (vira pontos e zera, é
...
**Empate exato** (mesmos pontos, saldo e vitórias) gera **co-campeões**. Uma
...
1. **Por rodada** — errado: a quadra travada estava na rodada 2 enquanto as
   presos ainda jogavam a rodada 1, então detectava zero conflito. (Hoje nem
...
do sorteio) e o "livre / em quadra" na troca de jogadores.
...
certa — 3 quadras exigem 12 jogadores **ao mesmo tempo**. O problema era de
comunicação, então a correção foi um **aviso bem visível** explicando a conta e
quantos jogadores faltam. Não mexer na lógica achando que é bug.

## "Não consegue abrir o app"

Investigado: site respondendo 200, deploys verdes, todos os arquivos no ar. Era
**cache velho no navegador dos jogadores**. Correções: tela de resgate no
...
- **Apagar um play** tira os pontos dos jogadores do ranking do mês e pode
  derrubar sequências. Um "ok" não basta: a tela mostra quantas partidas serão
  perdidas, **quantos pontos cada um perde**, avisa se o play já foi finalizado
  (o que refaz as sequências) e **exige escrever APAGAR**.
- **Fechar o mês** agora é um botão, não só a virada do calendário: a premiação
  acontece na última semana. A confirmação lista quem fecha com status e quanto
...
- **Trocar jogador não é mais um toque no nome.** O nome ficava dentro de uma
  linha cortada por reticências, então com nome comprido não dava para acertar o
  dedo. Agora é um botão "🔄 Trocar jogador" que abre um modal de duas etapas
...
Cada jogador tem um **id próprio** e as partidas guardam esse id, nunca o nome
...
- `name` é o **nome de cadastro**, completo. Serve para conferir a lista sem
  confundir duas Anas, e só aparece na tela de Jogadores.
...
A importação da lista também casa pelo apelido, senão o jogador que o grupo
escreve como "Aninha" viraria um segundo cadastro.

O que existe hoje para manter o cadastro único:

- **Editar perfil** — trocar o nome de cadastro, o apelido e os outros grafias.
...
- [ ] **Identidade visual.** A paleta atual é provisória e neutra — azul do mar,
      ...
      cores é mexer nesse bloco e mais nada.
- [ ] **Logo.** Enquanto `public/logo.png` não existir, o selo desenhado em
      `Logo()` assume. Soltar o arquivo lá é o bastante. Os ícones do PWA
      (`public/icon-*.png`) também são provisórios.
- [ ] **Nome definitivo.** "Pelada Semanal" é rótulo de trabalho. Está no
      `index.html`, no `public/manifest.webmanifest` e no `src/App.tsx`.
- [ ] **Artes de fechamento.** Foram removidas de propósito: as do original
      eram da identidade antiga. Quando houver identidade, dá para trazer o
      gerador de volta do repositório original (`src/lib/poster.ts`).
- [ ] **Dia da semana.** O original era às segundas; aqui ficou "semanal", sem
      dia fixo. Se o grupo fechar um dia, vale escrever nos textos.
