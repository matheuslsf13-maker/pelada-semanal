...
function PlayList({
  ...
  return (
    <>
      {canEdit && (
        <button className="btn marca block" style={{ marginBottom: 14 }} onClick={onNew}>
          🎾 Novo Play
        </button>
      )}
      <div className="card">
                ...
                <div key={s.id} className="row" style={{ borderBottom: '1px dashed var(--line)', paddingBottom: 10 }}>
                  <div className="grow" onClick={() => onOpen(s.id)} style={{ cursor: 'pointer', minWidth: 0 }}>
                    ...
                    <div className="tiny muted">
                      {dateLabel(s.date)} · {s.player_ids.length} jogadores · {s.courts} quadras
...
/**
 * Apagar um play tira os pontos dos jogadores do ranking do mes e pode derrubar
      ...
      {perdas.length > 0 && (
        <>
          <p className="tiny muted" style={{ marginBottom: 6 }}>
            {avulso
              ? 'Este play é avulso, então nada sai do ranking do mês — mas estes pontos somem do histórico dos jogadores:'
          ...
          <div className="stack" style={{ marginBottom: 12 }}>
            {perdas.slice(0, 5).map((s) => (
              <div key={s.player_id} className="row tiny" style={{ gap: 8 }}>
                ...
                <strong style={{ color: 'var(--marca)' }}>−{s.points} pts</strong>
              </div>
            ))}
            {perdas.length > 5 && (
...
function NewPlay({
   ...
   *
   * O app so conhece a forca de quem ja jogou; organizador conhece o resto.
  ...
  // mesmo grupo, entao dois grupos de 6 (12 jogadores) enchem duas quadras e nao
  ...
  const parceirosMin = Math.min(...tamanhos.map(parceirosDoRodizio))
  const parceirosMax = Math.max(...tamanhos.map(parceirosDoRodizio))
  // jogos e parceiros nao sao a mesma coisa: num grupo de 6 cada um joga com
  // os outros 5 e ainda repete uma, entao sao 6 jogos para 5 parceiros
  ...
  async function create() {
    ...
    if (selected.length < 4) {
      onToast('Precisa de pelo menos 4 jogadores')
      return
    }
    setBusy(true)
    try {
      ...
      const session: PlaySession = {
        id: uid(),
        date,
        title: title.trim() || 'Pelada Semanal',
        courts: effCourts,
  ...
  return (
    <>
      ...
      <div className="card">
        ...
        <div className="stack" style={{ marginTop: 12 }}>
          ...
          <div className="grid2">
            ...
            <div className="field">
              ...
              <em className={`hint${selected.length >= 4 && effCourts < courts ? ' aviso' : ''}`}>
                {selected.length < 4
                  ? 'cada quadra comporta 4 jogadores por vez'
                  : effCourts < courts
              ...
              <span className="grow">
                ...
                <span className="hint" style={{ marginTop: 2 }}>
                  {ranked
                    ...
                    : 'não soma pontos no ranking do mês e não mexe nas sequências 🔥 — mas conta no histórico do jogador e no equilíbrio das duplas dos próximos plays'}
                </span>
              </span>
            </label>
          </div>
          ...
          <p className="tiny muted" style={{ margin: '2px 2px 0' }}>
            {selected.length < 4 ? (
              'Escolha os jogadores abaixo para o app calcular as partidas.'
            ) : (
              <>
                <strong>{totalPartidas} partidas</strong> no total, entrando conforme as quadras vagam.
                Cada um faz dupla com{' '}
                <strong>
                  {parceirosMin === parceirosMax
                    ? `os outros ${parceirosMin}`
                    : `${parceirosMin} a ${parceirosMax} parceiros`}
                </strong>
                , exatamente uma vez com cada, e joga{' '}
                <strong>
                  {jogosMin === jogosMax ? `${jogosMin} partidas` : `${jogosMin} a ${jogosMax} partidas`}
                </strong>{' '}
                — o mesmo tanto para todos.
                {repetem > 0 && (
                  <>
                    {' '}
                    Como as duplas não fecham em partidas inteiras, cada um repete{' '}
                    <strong>{repetem === 1 ? '1 parceiro' : `${repetem} parceiros`}</strong> — a
                    mesma quantidade para todos, para ninguém jogar a mais que os outros.
                  </>
                )}
              </>
            )}{' '}
          ...
          {selected.length >= 4 && effCourts < courts && travadoPorGrupo && (
            <div className="banner err" style={{ margin: '10px 0 0' }}>
              ...
              Cada partida precisa de <strong>quatro jogadores do mesmo grupo</strong>, então um grupo
          ...
          {selected.length >= 4 && effCourts < courts && !travadoPorGrupo && (
            <div className="banner err" style={{ margin: '10px 0 0' }}>
              🏐 <strong>Não dá para usar {courts} quadras com {selected.length} jogadores.</strong>{' '}
              Cada quadra ocupa 4 jogadores ao mesmo tempo, então {courts} quadras precisam de{' '}
              <strong>{courts * 4} jogadores</strong> jogando juntas —{' '}
            ...
            <div className="banner warn" style={{ margin: '10px 0 0' }}>
              ...
              {grupos.length > 1 && tamanhos.every((t) => t === 4) ? (
                <>
                  ...
                  jogadores, jogando as 3 partidas do grupo uma atrás da outra. Cada grupo fica na sua
                ...
                <>
                  ...
                  quatro jogadores da próxima partida ainda estão jogando.
                </>
              )}
              <br />
              {grupos.length > 1 ? (
                <>
                  <strong>Grupos de 5, 9 ou 13</strong> são os melhores dos dois mundos: todos jogam
                ...
                <>
                  Com pelo menos <strong>4 de folga</strong> ({effCourts * 4 + 4} jogadores para{' '}
          ...
          {format === 'todos' && totalPartidas > 40 && (
            <div className="banner warn" style={{ margin: '10px 0 0' }}>
              ⏱️ São <strong>{totalPartidas} partidas</strong> e cada um joga {jogosMax} vezes —
              pode ser longo para uma noite só. O modo <strong>em grupos</strong> resolve isso:
              com grupos de 8 cada jogador joga 7 partidas.
            </div>
          )}
          ...
          {(format === 'grupos' || emDuplas) && grupos.length > 1 && (
            <div className="stack" style={{ marginTop: 4 }}>
              {grupos.map((g, i) => (
                <div key={i} className={`grupo-box ${classeDoGrupo(i + 1)}`}>
                  <div className="grupo-nome">
                    Grupo {i + 1} · {g.length} jogadores · {partidasDoRodizio(g.length)} partidas
                  </div>
              ...
              <em className="hint">
                Toque num jogador e escolha o grupo para onde ele vai. Quem já jogou entra pela
                força; quem está empatado (os estreantes) é sorteado, e cada “sortear de
      ...
      <button className="btn marca block" disabled={selected.length < 4 || busy} onClick={() => void create()}>
...
function descreverPodios(tamanhos: number[]): string {
  ...
  const jogadores = tamanhos.reduce((t, v) => t + v, 0)
  const chance = Math.round((100 * total) / jogadores)
  const iguais = tamanhos.every((t) => t === tamanhos[0])
  const detalhe = iguais ? `${vagas[0]} de cada ${tamanhos[0]}` : `${total} das ${jogadores}`
  return `${detalhe} (${chance}%)`
}

/** Devolve a partida com um jogador trocada por outra. */
...
function PlayDetail({
  ...
  const minutosMedios = (ms: Match[]) =>
    ms.length === 0
      ? minutosDaPartida(session.target, lerRegra(session.desempate))
  ...
  const jogosPorPessoa = useMemo(() => {
    ...
    for (const m of matches) for (const id of jogadoresDaPartida(m)) if (n.has(id)) n.set(id, (n.get(id) ?? 0) + 1)
    const v = [...n.values()]
  ...
  const ocupados = useMemo(() => {
    ...
    for (const m of emJogo) for (const id of jogadoresDaPartida(m)) s.add(id)
    return s
  }, [emJogo])

  /** Quantas partidas cada um ja fez hoje. */
  const jogos = useMemo(() => {
    const map = new Map<string, number>()
    for (const m of jogadas) for (const id of jogadoresDaPartida(m)) map.set(id, (map.get(id) ?? 0) + 1)
    return map
  }, [jogadas])

  /**
   * Fila de espera: 0 e quem esta fora ha mais tempo. Sem hora registrada
   * (banco antigo, outro aparelho) o jogador conta como "jogou ha muito".
   */
  const espera = useMemo(() => {
    ...
    for (const m of jogadas) {
      ...
      for (const id of jogadoresDaPartida(m)) {
  ...
  const livresAgora = useMemo(() => {
    const comprometidas = new Set(ocupados)
    for (const m of proximas.values()) for (const id of jogadoresDaPartida(m)) comprometidas.add(id)
    return session.player_ids.filter((id) => !comprometidas.has(id))
  }, [ocupados, proximas, session.player_ids])
  ...
  const filaPrevista = useMemo(() => {
    ...
    for (const m of proximas.values()) {
      for (const id of jogadoresDaPartida(m)) comprometidas.add(id)
    }
    return ordemPrevista({
      pendentes: pendentes.filter((m) => !naQuadra.has(m.id)),
      espera,
      ocupados: comprometidas,
      jogadores: session.player_ids,
    })
  }, [pendentes, proximas, ocupados, espera, session.player_ids])

  /**
   * As duplas que jogam duas vezes no dia, por partida.
   *
   * Nao e sobra nem defeito: quando o grupo nao fecha certo (6, 7, 10, 11
   * jogadores) algumas duplas repetem DE PROPOSITO, escolhidas para que cada
   * jogador repita a mesma quantidade -- sem isso duas do grupo jogariam uma
   * partida a mais que os outros.
   */
  ...
  async function substituir(p: {
    ...
    if (sai && matches.some((m) => iniciada(m) && jogadoresDaPartida(m).includes(sai))) {
      onToast(`${nameOf(sai)} está em quadra agora — lance o placar antes de tirá-lo`)
      return
    }

    // quem entra: cadastrada, ou criada agora (reaproveitando se o nome ja existe)
    let entra: string | null = null
    // o nome de quem entra: quem e criado agora ainda nao esta no `data`
    ...
    if (groups && entra) {
      ...
      if (onde === 'lugar' && sai) {
        ...
        // o grupo cuja forca media fica mais perto da dele
    ...
    if (onde === 'lugar' && sai && entra) {
      const trocadas = matches
        .filter((m) => !isPlayed(m) && jogadoresDaPartida(m).includes(sai))
    ...
    <>
        ...
        <div style={{ marginTop: 10 }}>
          ...
          {matches.length > 0 && (
            <div className="tiny" style={{ marginTop: 6, fontWeight: 700 }}>
              🎾 Cada jogador joga <strong>{jogosPorPessoa}</strong>
              {' · '}
              {finished
      ...
      {session.ranked === false && (
        <div className="banner info">
          ...
          jogador e continuam ajudando a equilibrar as duplas dos próximos plays.
        </div>
      )}
      ...
      <div className="card">
        ...
        {jaClassificadas && (
          <div className="banner ok classificados">
            🎟️ <strong>
          ...
          quadras.map((q) => {
            ...
            return (
              <MatchCard
                ...
                jogadoresDoPlay={session.player_ids}
              />
            )
          })
        )}
      ...
      {showRank && (
        <Modal title={`Ranking do dia — ${dateLabel(session.date)}`} onClose={() => setShowRank(false)}>
          {dayRows.length === 0 ? (
            ...
            <>
              {award && awardLevel && (
                <div className="banner warn" style={{ background: 'var(--bronze-suave)', color: 'var(--bronze)' }}>
                  ...
                  <strong>{award.value} pontos</strong>, que ele decide se usa no fechamento do mês.
                  ...
                  return (
                    <div key={p.grupo} style={{ marginBottom: 14 }}>
                      <div className="section-title" style={{ fontSize: 13 }}>
                        👥 Grupo {p.grupo}{' '}
                        <span className="tiny muted" style={{ fontWeight: 600 }}>
                          · {p.membros.length} jogadores · pódio: {p.rows.length}
                        </span>
                      </div>
                      <RankTable
              ...
              {podios.length > 1 && (
                <>
                  <div className="section-title" style={{ fontSize: 13 }}>📊 Todos juntos</div>
                  <p className="tiny muted" style={{ marginTop: 0 }}>
 ...
 * mesmo jogador em duas quadras, a tela diz o que esta travando e oferece
...
function QuadraEsperando({
  ...
  livres: string[]
  editable: boolean
  /** Grupo de cada jogador, quando o play e em grupos. */
  ...
  return (
    <div className="match vazia">
      ...
      {restam === 0 ? (
        ...
        <>
          <div className="tiny muted">
            Nenhuma das {restam} partidas que faltam tem quatro jogadores livres agora.
          </div>
          {livres.length > 0 ? (
            <>
              {emGrupos ? (
                <>
                  ...
                  <div className="tiny muted" style={{ marginTop: 4 }}>
                    Uma partida precisa de <strong>quatro do mesmo grupo</strong> — juntar jogadores de
              ...
              {editable && (
                <div className="tiny muted" style={{ marginTop: 6 }}>
                  A quadra espera a próxima partida terminar — trocar jogadores aqui desfaria uma dupla
                  do rodízio e daria um jogo a mais para uma e a menos para outra. Se alguém
                  <strong> precisou ir embora</strong>, troque o jogador direto no card da partida.
                </div>
              )}
            </>
          ) : (
            <div className="tiny muted" style={{ marginTop: 6 }}>
              Todas os jogadores estão em quadra. Assim que um placar for lançado, esta quadra recebe
              a próxima partida.
            </div>
          )}
        </>
...
function Situacao({
  ...
  const texto = jogando.has(id) ? 'em quadra' : ocupados.has(id) ? 'próxima partida' : 'livre'
  const cor = jogando.has(id) ? 'var(--bronze)' : ocupados.has(id) ? 'var(--apoio)' : 'var(--verde)'
  return (
...
function MatchCard({
  ...
  iniciada: boolean
  inicio: string | null
  /** Indisponiveis: jogando agora ou ja escalados para outra quadra. */
  ocupados: Set<string>
  ...
  const modalTroca = trocando && (
    <TrocarJogadores
      ...
      jogando={jogando}
      espera={espera}
      jogadoresDoPlay={jogadoresDoPlay}
      ...
      <span>
        ...
        {repetida && (
          <span
            ...
            title="uma das duplas joga pela segunda vez, para todos fecharem com o mesmo número de partidas"
          >
            🔁
          </span>
        )}
    ...
    <div className={`match live ${corDoGrupo}${iniciada ? ' em-quadra' : ''}`}>
      ...
      {iniciada ? (
        <div className="row spread" style={{ marginBottom: 8 }}>
          <span className="tiny" style={{ fontWeight: 800, color: 'var(--verde)' }}>
        ...
        <button className="btn verde sm block" style={{ marginBottom: 8 }} onClick={onIniciar}>
          ▶️ Partida iniciada
        </button>
      )}
      ...
      */}
      <button className="pick-team" disabled={!iniciada} onClick={() => setWinner('a')}>
        <Duo ids={match.team_a} ocupados={iniciada ? undefined : ocupados} />
        <span className="pick-tag">venceu</span>
      </button>
      <div className="vs">X</div>
      <button className="pick-team" disabled={!iniciada} onClick={() => setWinner('b')}>
        <Duo ids={match.team_b} ocupados={iniciada ? undefined : ocupados} />
        <span className="pick-tag">venceu</span>
      </button>
      {!iniciada && (
      ...
      <div className="row" style={{ gap: 8, marginTop: 8 }}>
        <button className="btn ghost sm grow" onClick={() => setTrocando(true)}>🔄 Trocar jogador</button>
...
function ListaDePartidas({
  ...
  numerar?: boolean
  /** Quantas partidas cada jogador ja fez hoje. */
                ...
                <span className="grow" style={{ minWidth: 0 }}>
                  ...
                  {repetidas.has(m.id) && (
                    <span className="tiny muted">
                      ...
                      — é o que deixa todos com o mesmo número de partidas
                    </span>
                  )}
                  {novatos.length > 0 && (
                    <span className="tiny" style={{ color: 'var(--verde)', fontWeight: 700 }}>
                      🆕 {novatos.length === 4
                        ? 'os quatro ainda estão esperando a primeira partida'
                        : `${novatos.map(nameOf).join(', ')} ainda não ${novatos.length === 1 ? 'entrou' : 'entraram'} em quadra`}
                    </span>
                  )}
                  {!jogada && jogadoresDaPartida(m).some((id) => emQuadra.has(id)) && (
...
function CorrigirPlacar({
  ...
  // o tie tem placar proprio, entao ele e um passo a parte tambem aqui
  if (winner && noTie) {
      ...
      <Modal title="Quantos pontos no tie?" onClose={onClose}>
        ...
        <div className="games-row">
          {pontosDoPerdedorNoTie(desempate).map((p) => (
            ...
            >
              {pontosDoVencedorNoTie(desempate, p) === desempate.tie ? p : placarDoTie(desempate, p)}
            </button>
          ))}
          <button className="game-btn manual" onClick={() => setDigitando(true)}>✏️</button>
        </div>
        ...
        {digitando && (
          <PlacarManual
            ...
            onConfirmar={(_v, perdeu) =>
              winner === 'a'
                ? onScore(noTie.venceu, noTie.perdeu, perdeu)
                : onScore(noTie.perdeu, noTie.venceu, perdeu)
            }
          />
        )}
      </Modal>
    ...
    return (
      <Modal title="Quantos games o perdedor fez?" onClose={onClose}>
        ...
        {digitando && (
          <PlacarManual
            vencedora={(winner === 'a' ? match.team_a : match.team_b).map(nameOf).join(' + ')}
            perdedora={perdedors.map(nameOf).join(' + ')}
            valida={(a, b) => placarDeGamesValido(target, desempate, a, b)}
            explica={(a, b) => explicarGamesInvalido(target, desempate, a, b)}
            onCancelar={() => setDigitando(false)}
            onConfirmar={(venceu, perdeu) =>
              winner === 'a' ? onScore(venceu, perdeu) : onScore(perdeu, venceu)
            }
          />
        )}
      </Modal>
...
/**
 * Troca de jogador em duas etapas, com linhas grandes.
...
function TrocarJogadores({
  ...
  const candidatas = jogadoresDoPlay
    .filter((id) => !noTime.includes(id))
    .sort((a, b) => {
      const oa = ocupados.has(a) ? 1 : 0
      const ob = ocupados.has(b) ? 1 : 0
        ...
        <div className="stack">
          {noTime.map((id) => (
            <button key={id} className="duo-row" onClick={() => setSai(id)}>
              ...
              {ocupados.has(id) && <Situacao id={id} ocupados={ocupados} jogando={jogando} />}
            </button>
          ))}
        </div>
      </Modal>
  ...
  return (
    <Modal title={`Quem entra no lugar de ${nameOf(sai)}?`} onClose={onClose}>
      ...
      {candidatas.length === 0 ? (
        <Empty icon="👥">Todas os jogadores já estão nesta partida.</Empty>
      ) : (
        <div className="stack">
          {candidatas.map((id) => (
            <button key={id} className="duo-row" onClick={() => onTrocar(sai, id)}>
              <Avatar player={playerById(id)} size={38} />
              <span className="grow ellipsis" style={{ fontWeight: 700 }}>{nameOf(id)}</span>
              <Situacao id={id} ocupados={ocupados} jogando={jogando} />
            </button>
          ))}
        </div>
      )}
...
function EscolherPartida({
  ...
  const ordenadas = [...partidas].sort((a, b) => {
    const la = jogadoresDaPartida(a).every((id) => !ocupados.has(id)) ? 0 : 1
    const lb = jogadoresDaPartida(b).every((id) => !ocupados.has(id)) ? 0 : 1
    const ea = jogadoresDaPartida(a).reduce((t, id) => t + (espera.get(id) ?? 0), 0)
    const eb = jogadoresDaPartida(b).reduce((t, id) => t + (espera.get(id) ?? 0), 0)
  ...
  return (
    <Modal title={`Qual partida entra na quadra ${quadra}?`} onClose={onClose}>
      <p className="tiny muted" style={{ marginTop: 0 }}>
        As de cima são as que têm as quatro jogadores livres e esperando há mais tempo.
      </p>
      <div className="stack">
        {ordenadas.slice(0, 30).map((m) => {
          const presos = jogadoresDaPartida(m).filter((id) => ocupados.has(id))
          return (
            <button key={m.id} className="duo-row" onClick={() => onEscolher(m)} disabled={presos.length > 0}>
              ...
              <span className="grow" style={{ minWidth: 0 }}>
                ...
                {presos.length > 0 && (
                  <span className="tiny" style={{ color: 'var(--bronze)' }}>
                    ⏳ {presos.map(nameOf).join(', ')} em quadra
                  </span>
                )}
              </span>
            </button>
    ...
    <Modal title="🔁 Entra / sai" onClose={onClose}>
        ...
        {modo === 'nova' && (
          <>
            ...
            <em className="hint" style={{ marginTop: 6 }}>
              Entra no cadastro agora, como isento e com a força inicial padrão — dá para ajustar
              depois no perfil.
            </em>
          </>
        )}
            ...
            <label className={`fase-box${!podeRefazer ? ' off' : ''}`} style={{ gap: 8 }}>
              ...
              <span>
                ...
                <span className="tiny muted">
                  ...
                  não aconteceu é refeita com ele
                </span>
              </span>
            </label>
            {grupos.length > 1 && (
                ...
                <span className="grow">
                  ...
                  {onde === 'grupo' && (
                    <select className="select" style={{ marginTop: 6 }} value={grupo} onChange={(e) => setGrupo(Number(e.target.value))}>
                      {grupos.map((g, i) => (
                        <option key={i} value={i}>Grupo {i + 1} · {g.length} jogadores</option>
                      ))}
                    </select>
                  )}
                </span>
      ...
      {!temEntra && temSai && emFase2 && (
        <div className="banner warn" style={{ marginTop: 8 }}>
          Com as duplas fixas formadas, alguém precisa entrar no lugar dele — senão a dupla fica sem par.
        </div>
      )}
