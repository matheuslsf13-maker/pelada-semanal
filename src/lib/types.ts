export type Player = {
  ...
  created_at: string
  /**
   * Como ele aparece na quadra: nos rankings, nas partidas, nos textos e nas
  ...
  /** Outras grafias que a lista do grupo ja usou para esse jogador. */
  aliases?: string[]
  /**
   * Como ele paga: `mensalista`, `avulso` ou `convidado`.
   * Ver `src/lib/mensalidade.ts` -- e la que mora a regra de quem pode entrar.
   */
  categoria?: string | null
  /** Mensalista: ate que mes esta pago (AAAA-MM). */
  pago_mes?: string | null
  /** Avulso: credito de UMA participacao, gasto quando o play e finalizado. */
...
/**
 * Formato do dia:
 *  - 'todos'  : rodizio unico, cada um faz dupla com cada um dos outros;
...
export type Match = {
   ...
   * senao a fase da final teria duas partidas, e o app usa esse numero
   * para saber qual rodada e (1 jogo = final, 2 = semifinal).
   */
...
/**
 * Escolha do jogador no fechamento do mes, quando ele esta em chamas:
...
/**
 * Fechamento de mes feito na mao pela organizador ("finalizar o mes").
