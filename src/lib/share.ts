 ...
 *
 * Quando o jogador esta em chamas, o degrau vai junto na semana linha -- o
 * emoji sozinho nao diz se ele esta "Em chamas" ou "Craque da temporada".
 */
...
export type DayTextOpts = {
   ...
   */
  duplas?: DuplaDoDia[]
  /** Sequencia de cada jogador depois deste play (so quem tem 2 ou mais). */
...
export function dayRankingText(opts: DayTextOpts): string {
  ...
  if (duplas && duplas.length > 0) {
    const titulos = ['*Campeões do dia*', '*Vice-campeões*', '*3º lugar*']
    partes.push(
      '\n' +
        duplas
          .slice(0, 3)
    ...
    if (fora.length > 0) {
      ...
      // de que grupo cada um veio -- e o que explica quem subiu com quantos pts
      partes.push(
        '\n*Demais jogadores*\n' +
          fora
  ...
  if (award && lvl) {
    partes.push(
      `\n${lvl.emoji} *${nameOf(award.player_id)} é ${lvl.title.toUpperCase()}!*\n` +
        ...
        `que ele decide se usa no fechamento do mês.`,
    )
  }
