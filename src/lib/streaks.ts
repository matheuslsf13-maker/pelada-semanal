 ...
 *
 * Como as duplas sao equilibradas, a campeao do dia e quase sorteio: medindo em
 ...
 *
 * O status vale pontos uma unica vez: no fechamento do mes, se ele ainda o
 * tiver, escolhe USAR (os pontos entram naquele mes e o status zera) ou
 * PRESERVAR (nao pontua, o status segue e cresce, e ele ganha a vida).
 *
 * O mes fecha de tres jeitos: quando aparece um play de um mes seguinte,
 * quando o calendario passa do mes, ou quando a organizador aperta
...
export const STREAK_LADDER = [
  ...
  { from: 5, to: 5, emoji: '👑🔥', title: 'Lenda da pelada', value: 16 },
  { from: 6, to: 6, emoji: '👑💎', title: 'Rei da quadra', value: 24 },
  { from: 7, to: 7, emoji: '👑🌟', title: 'Imperador', value: 34 },
  { from: 8, to: null, emoji: '👑💎🌟', title: 'Craque da temporada', value: 50 },
] as const

/** Quantos pontos vale o status que ele tem agora. */
...
/** Quantos jogadores sobem ao podio -- de cada grupo, quando ha grupos. */
...
export type PodioDoDia = {
  /** Numero do grupo (1, 2, 3...) ou null quando o play e todos com todos. */
  grupo: number | null
  /** As primeiras do grupo, com empate exato na ultima vaga subindo junto. */
  rows: PlayerStat[]
  /** Todas os jogadores do grupo, para saber de onde vem quem ficou de fora. */
  membros: string[]
}
...
/** Pontos creditados num mes porque o jogador decidiu usar o status. */
...
export type Streaks = {
  ...
  /** Vidas disponiveis de cada jogador (0 ou 1). */
...
export function computeStreaks(data: AppData): Streaks {
  ...
  for (const s of finished) {
    ...
    // campeoes do dia (para os titulos e a arte do mes)
    const top = rank[0]
  ...
  // O ultimo mes fecha quando a organizador aperta "finalizar o mes" ou,
...
/** Jogadores com status vivo, da maior sequencia para a menor. */
