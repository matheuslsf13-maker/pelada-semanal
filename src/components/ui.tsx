 ...
 * Selo do campeonato.
 *
 * PROVISORIO: o grupo ainda nao tem identidade. Enquanto `public/logo.png` nao
 * existir, este desenho assume; basta soltar o logo novo nesse caminho e nada
 * mais precisa mudar.
 */
export function Logo({ size = 64 }: { size?: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" style={{ flex: 'none' }} aria-label="Pelada Semanal">
      <defs>
        <linearGradient id="selo" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0" stopColor="#3f5a8a" />
          <stop offset=".55" stopColor="#0d7c8a" />
          <stop offset="1" stopColor="#16794f" />
        </linearGradient>
      </defs>
      <circle cx="50" cy="50" r="47" fill="url(#selo)" />
      {/* bola de beach tennis, com a costura em arco */}
      <circle cx="50" cy="47" r="26" fill="#0c151a" opacity=".18" />
      <circle cx="50" cy="45" r="26" fill="#f2f7f9" />
      <path
        d="M31 30c11 8 11 22 0 30M69 30c-11 8-11 22 0 30"
        stroke="#0d7c8a"
        strokeWidth="3.4"
        fill="none"
        strokeLinecap="round"
