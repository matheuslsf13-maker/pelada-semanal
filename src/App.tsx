...
export default function App() {
  ...
  const [abrirStats, setAbrirStats] = useState<'jogador' | 'duplas' | 'forca' | null>(null)
    ...
    <div className="app">
      <header className="topbar">
        <div className="row spread">
          <Logo size={46} />
          <div className="grow">
            <h1>Pelada <span>Semanal</span></h1>
            <div className="sub">
          ...
          {online && (
            userEmail
              ...
              : <button className="btn marca sm" onClick={() => setLogin(true)}>Entrar</button>
          )}
        </div>
      </header>
      ...
      {online && !canEdit && (
        <div className="banner warn row spread" style={{ gap: 12 }}>
          ...
          <button className="btn marca sm nowrap" onClick={() => setLogin(true)}>🔑 Entrar</button>
        </div>
      )}
      {!online && (
        <div className="banner warn">
...
function LoginModal({
  ...
  return (
    <Modal title="Entrar como organizador" onClose={onClose}>
      <div className="stack">
        ...
        <button className="btn marca block" disabled={busy || !email || !password} onClick={() => void submit()}>
          {busy ? 'Entrando…' : 'Entrar'}
        </button>
        <p className="tiny muted" style={{ margin: 0 }}>
          Só quem organiza precisa entrar. As outros jogadores abrem o link e já veem o ranking.
        </p>
      </div>
    </Modal>
  )
