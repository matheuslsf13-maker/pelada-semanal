# 🏐 Pelada Semanal — Beach Tennis

App para organizar o **Pelada Semanal**, o campeonato de **beach tennis** do grupo: monta as duplas equilibradas,
...
- **Rodízio completo** — cada jogador faz dupla com **cada um dos outros
  exatamente uma vez**. As duplas são montadas pela pontuação de cada jogador
...
- **Sem rodadas: uma fila de partidas** — as quadras nunca terminam juntas, então
  ...
  próxima partida cujas quatro jogadores estão livres, **dando preferência a quem
  está fora há mais tempo** — ninguém emenda dois jogos cansada enquanto outra
  espera sentada.
- **Dois formatos** — *Todos com todos* (o rodízio inteiro) ou **em grupos**:
  você escolhe quantos jogadores por grupo e o app monta os grupos **por nível**
  ...
  Serve para noite curta: com 16 jogadores o rodízio inteiro dá 15 jogos para cada
  uma; em grupos de 8, dá 7.
- **Quantidade livre** — você informa quantos jogadores vieram e quantas quadras
...
- **Status de sequência 🔥** — terminar semanas seguidas no **pódio do dia** dá
  status, de 🔥 *Em chamas* (2 semanas) até 👑💎🌟 **Craque da temporada** (8 ou mais).
  No fechamento do mês o jogador escolhe **usar** o status (vira pontos e zera)
  ou **preservar** (segue crescendo e ganha 1 vida).
- **Fechar o mês quando você quiser** — botão com confirmação no Ranking, porque
  a premiação acontece na última semana e não na virada do calendário. Dá para
...
- **Play avulso** — na criação dá para marcar que o play **não vale para o
  campeonato** (aquele jogo de segunda). Ele conta no histórico do jogador e
...
- **Estatísticas** — em dois modos. *Por jogador*: aproveitamento, saldo,
  ...
  completo de cada um — inclusive contra quem jogaram e o placar de cada partida.
...
- **Imagem de fechamento do mês** — gera a arte do pódio (1080×1350, formato de
  post/status) com as fotos do top 3, coroa no campeão, pontos, vitórias e as
  demais colocadas. Um toque para compartilhar ou salvar.
- **Fotos** — cada jogador pode ter foto de perfil (aparece no pódio e na arte
...
**todas os outros só abrem o link** e já veem o ranking atualizado em tempo real.
...
- **Uma quadra por vez** — nada de rolar 28 partidas procurando a atual. A tela
  ...
  logo abaixo.
- **Fora do roteiro também funciona** — *Trocar jogador* abre uma lista de
  ...
  quatro jogadores livres o app diz quem está livre e monta uma partida com elas.
...
WhatsApp do jeito que ela vem, numerada e com a grafia de cada um:
...
Ao confirmar, as grafias usadas ficam guardadas como **apelidos** do jogador,
então na semana seguinte "Carol", "Tete" e "Gabi" são reconhecidas sozinhas.
Logo depois da importação há um botão para **desfazer** tudo, e se um atleta
repetida escapar, a aba **Jogadores** tem o 🔗 para **juntar dois jogadores** —
...
1. **Jogadores** — cadastre os jogadores (e as fotos).
2. **Play → Novo Play** — escolha data, quadras, o formato (todos com todos ou
...
Terminar a semana no **pódio do dia** (top 3) mantém o status vivo e faz a
sequência crescer:
...
| 5 | 👑🔥 Lenda da pelada | 16 pontos |
| 6 | 👑💎 Rei da quadra | 24 pontos |
| 7 | 👑🌟 Imperador | 34 pontos |
| **8 ou mais** | **👑💎🌟 Craque da temporada** | **50 pontos** |

### Por que pódio e não vitória do dia

Porque as duplas são equilibradas de propósito: medindo 400 semanas simuladas,
**a mesmo jogador vence duas semanas seguidas em apenas 11% das vezes** e
...
alcançável e a Craque continua lendária (cerca de 1,5% das sequências).
...
A **vida** absorve uma semana fora do pódio: o status sobrevive, mas não cresce
...
**Os parceiros** saem do *método do círculo* (uma 1-fatoração do grafo completo):
fixa um jogador e gira os outros, gerando conjuntos de duplas em que ninguém se
repete e toda combinação aparece uma única vez. Quando o total de combinações é
ímpar (9, 10, 11, 14, 15, 18… jogadores) uma dupla precisa jogar duas vezes,
...
**A força de cada jogador** é um **Elo**: cada partida move a nota das quatro
...
senão a mesmo jogador acaba sugerida em duas quadras ao mesmo tempo. A lista
...
> número de jogadores, ninguém fica de fora e a quadra que terminar primeiro
> espera os outros. Com pelo menos 4 de folga o rodízio anda sozinho — o app
...
src/pages/           Ranking · Play · Estatísticas · Jogadores
